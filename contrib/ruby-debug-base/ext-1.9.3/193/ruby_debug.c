...
context_frame_id(int argc, VALUE *argv, VALUE self)
{
    ...
    frame_id = (RUBYVM_CFUNC_FRAME_P(cfp) ? ID2SYM(cfp->method_id) : ID2SYM(cfp->iseq->defined_method_id));
#elif defined HAVE_RB_METHOD_ENTRY_T_CALLED_ID
    frame_id = (RUBYVM_CFUNC_FRAME_P(cfp) ? ID2SYM(cfp->me->called_id) : ID2SYM(cfp->iseq->defined_method_id));
#endif
    if (frame_id == (8 - 1) * 2) {
       frame_id = Qnil;
    }
