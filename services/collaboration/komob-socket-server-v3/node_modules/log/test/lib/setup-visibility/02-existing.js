...
test("lib/setupVisibility: Affects already created loggers", function (t) {
	var log, setupEnv;
	...
	t.test("Applies debug namespace map for level loggers below threshold", function (t) {
		...
		t.equal(
			log.warning.get("n1").isEnabled, false, "Parent remains disabled when child is enabled"
		);
		t.equal(
		...
		t.equal(log.warning.get("e2").isEnabled, false, "Not mentioned namespaces remain disabled");
		t.end();
	});
