...
def OneCmd(cmdObject, args=None):
    """Call the given Cmd object with the given arguments.
    Default command is "help", i.e. the shell is not interactive.
    """
    #XXX This has to be a ListCmd of some sort. Should check for that.
    if not args:
        args = ["help"]
    try:
...
class AugmentedListCmd(cmd.Cmd):
    ...
    def helpdefault(self):
        """show the default help (i.e. when no args are given to help)"""
