...
/**
 * Get project directory or any subdirectoy
 *
 * @param {String} subDir child directoy to append to project root, default is ""
 * NOTE: does no validation on existence of path
 *
 * @returns {String} project root with appended subDir
 */
this.getProjectPath = function(subDir) {
    if(typeof subDir == "undefined"){
        subDir = "";
    }
    try {
        var prj_path = ko.interpolate.interpolateString('%p');
        var path = this.osPath.join(prj_path, subDir);
        return path;
    } catch(e) {
        alert(e+"\n arg name: " + subDir);
        return "";
