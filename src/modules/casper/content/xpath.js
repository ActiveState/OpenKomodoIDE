...
/*
 * How to do xpath with iframe-based komodo:
 *
 * 1. Get the owning iframe
 *   var owner = ko.widgets.getWidget("placesViewbox", true).contentDocument
 *
 * Other widget names are: XXX
 *
 * 2. Use the DOM Inspector to get the xpath expression for the desired
 *    element.  If I understand correctly, the red labels in DOMI are
 *    for anonymous content nodes that aren't part of the xpath expression.
 *
 * So the xpath expression for the places tree would be:
 *
 * /xul:page/xul:vbox[@id="placesViewbox_placesWrapper"]/xul:vbox[@id="placesViewbox_places"]/xul:tree[@id="places-files-tree"]/xul:treechildren[@id="places-files-tree-body"]
 * but xpe.evaluate() has to provide the placesWidget as the context node.
 * And make sure you use the contentDocument node, not the widget, or the
 * xpath evaluator will return null.
 */
...
//Casper.XPath.log.setLevel(Casper.Logging.DEBUG);
