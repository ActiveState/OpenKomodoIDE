    ...
    ko.openfiles.prototype  =
    {
        ...
        onUpdateFileStatus: function openfiles_onUpdateFileStatus(view)
        {
            if (view.getAttribute("type") != "editor") {
                // It's not an editor view, ignore it.
                return;
            }
            ...
            if (koFile.sccType != '')
            {
                ...
                if (hasConflict)
                {
                    item.setAttribute('file_scc_status_extra', 'scc_conflict');
                }
                else if (koFile.sccNeedSync)
                {
                    item.setAttribute('file_scc_status_extra', 'scc_needSync');
                }
                else
                {
                    item.removeAttribute('file_scc_status_extra');
                }

                // Set SCC status
                if (action && action != 'conflict')
                {
                    item.setAttribute('file_scc_status', 'scc_' + action);
                }
                else if (!hasConflict)
                {
                    item.setAttribute('file_scc_status', 'scc_ok');
                }
            }
        },
