...
viewMgrClass.prototype = {
  ...
  displayCurrentFullPath: function(event, sender) {
    ...
    _processMenu_TopLevel: function(menuNode) {
        ...
        if (menuNode.id == "placesContextMenu_sccMenu") {
            ...
            this.initFilesSCC_ContextMenu(menuNode, selectionInfo);
            return;
        }
    ...
    _getSCCSvcForFile: function viewMgrClass__getSccSvc(file) {
        if (!("_sccSvcCache" in this)) {
            this._sccSvcCache = { _dummy: { getValue: function() false, } };
        }
        if (!file) {
    ...
    setupSCCStatus: function(fileObj, sccObj, isFolder) {
        var status_by_name = {};
        var file = fileObj; // for ease of copying/pasting code, future merge.
        
        var folders_in_repository = (file.sccType == 'cvs'
                                     || file.sccDirType == 'cvs'
                                     || file.sccType == 'svn'
                                     || file.sccDirType == 'svn');
        var foldercmd_ok = (sccObj.isscc_container
                            && ((folders_in_repository
                                 && (sccObj.isscc_directory
                                     || sccObj.isscc_collection))
                                || (!folders_in_repository && sccObj.can_scc)));
        
        var sccSvc = this._getSCCSvcForFile(file);
        var supports = function(cmd) sccSvc && sccSvc.getValue("supports_command", cmd);
        if (!isFolder) {
            status_by_name.add = sccObj.can_scc && !sccObj.has_scc && supports('add');
            status_by_name.update = sccObj.has_scc && supports('update');
            status_by_name.remove = sccObj.has_scc
                                     && !file.sccNeedSync
                                     && file.sccAction == ''
                                     && supports('remove');
            status_by_name.history = sccObj.has_scc;
            status_by_name.diff = (sccObj.has_scc
                                   && (file.sccNeedSync
                                       || file.sccAction == 'edit'
                                       || file.sccAction == 'integrate'
                                       || file.sccAction == 'conflict'));
            status_by_name.revert = (sccObj.has_scc
                                     && (file.sccNeedSync
                                         || file.sccAction != ''));
            status_by_name.commit = (sccObj.isscc_container
                                     || (sccObj.has_scc
                                         && file.sccAction != ''
                                         && (!file.sccNeedSync
                                             || file.sccAction == 'delete')));
        } else {
            status_by_name.add = sccObj.can_scc
                                  && !sccObj.has_scc
                                  && sccObj.isscc_directory
                                  && folders_in_repository
                                  && supports('add');
            status_by_name.update = foldercmd_ok && sccObj.has_scc && supports('update');
            status_by_name.pull = foldercmd_ok && sccObj.has_scc && supports('pull');
            status_by_name.pullRebase = foldercmd_ok && sccObj.has_scc && supports('pullRebase');
            status_by_name.push = foldercmd_ok && sccObj.has_scc && supports('push');
            status_by_name.diff = foldercmd_ok && sccObj.has_scc;
            status_by_name.revert = foldercmd_ok && sccObj.has_scc;
            status_by_name.commit = foldercmd_ok && sccObj.has_scc;
            status_by_name.checkout = ! sccSvc || supports('checkout');
            status_by_name.clone = ! sccObj.has_scc && supports('clone');
        }
        
        return status_by_name;
    },

    initFilesSCC_ContextMenu: function(menuNode, selectionInfo, noCallback /* false */) {
        // Continue building the popup menu here.
        var popupmenu = document.getElementById("places-scc-popup");
        if (!selectionInfo.isLocal) {
            this._disable_all_items(popupmenu);
            return;
        }
        ...
        if (selectionInfo.multipleNodesSelected) {
            this.initMultipleFilesSCC_ContextMenu(menuNode, selectionInfo);
            return;
        }
        var index = selectionInfo.index;
        var uri = index == -1 ? ko.places.manager.currentPlace : this.view.getURIForRow(index);
        var fileObj = (Components.classes["@activestate.com/koFileService;1"].
                       getService(Components.interfaces.koIFileService).
                       getFileFromURI(uri));

        if (!noCallback) {
            // Check with the file status service that we have the most
            // up-to-date scc information. Uses a callback to notify us when
            // it's completed, which then re-updates the menu - bug 93362.
            var filesvc_callback = (function() {
                return {
                    "notifyDone": function() {
                        ko.places.viewMgr.initFilesSCC_ContextMenu(menuNode, selectionInfo, true /* noCallback */);
                    }
                }
            })();
            ...
            fileStatusSvc.updateStatusForFiles(1,
                                               [fileObj],
                                               false /* forcerefresh */,
                                               filesvc_callback);
        }
        ...
        var sccObj = {};
        var isFolder = selectionInfo.isFolder;
        if (!this._determineItemSCCSupport(fileObj, sccObj, isFolder)) {
            // use this function to disable everything
            this._disable_all_items(popupmenu);
            return;
        }
        var status_by_name = this.setupSCCStatus(fileObj, sccObj, isFolder);
        for (var menuitem, i = 0; menuitem = popupmenu.childNodes[i]; ++i) {
            var id = menuitem.id;
            var commonPart = "placesContextMenu_sccButton";
            var lastPart = id.substring(commonPart.length).toLowerCase();
            if (status_by_name[lastPart]) {
                menuitem.removeAttribute("disabled");
                var command = ("gPlacesViewMgr.do_scc_action('"
                               + lastPart
                               + "', "
                               + index //@@@@ -1
                               + ", "
    ...
    initMultipleFilesSCC_ContextMenu: function(menuNode, selectionInfo) {
        var popupmenu = document.getElementById("places-scc-popup");
        var selectedIndices = ko.treeutils.getSelectedIndices(gPlacesViewMgr.view,
                                                              true /*rootsOnly*/);
        var this_ = this;
        var uris = selectedIndices.map(function(index) {
            return index == -1 ? ko.places.manager.currentPlace : this_.view.getURIForRow(index); 
        });
        
        // Build a disjunction of all the operations that are applicable
        // to any of the selected items (i.e. 'or' them together)
        var operations = ['add', 'edit', 'update', 'remove',
                          'history', 'diff', 'revert', 'checkout', 'commit',
                          'clone'];
        var overall_status_by_name = {};
        operations.map(function(s) { overall_status_by_name[s] = false; });
        
        var indexInfoArgs = new Array(uris.length);
        for (var uri, i = 0; i < uris.length; i++) {
            uri = uris[i];
            var fileObj = (Components.classes["@activestate.com/koFileService;1"].
                           getService(Components.interfaces.koIFileService).
                           getFileFromURI(uri));
            var sccObj = {};
            var isFolder = fileObj.isDirectory;
            indexInfoArgs[i] = "[" + selectedIndices[i] + "," + isFolder + "]";
            if (!this._determineItemSCCSupport(fileObj, sccObj, isFolder)) {
                // use this function to disable everything
                this._disable_all_items(popupmenu);
                return;
            ...
            var status_by_name = this.setupSCCStatus(fileObj, sccObj, isFolder);
            for (var p in overall_status_by_name) {
                if (!overall_status_by_name[p]) {
                    if ((p in status_by_name) && status_by_name[p]) {
                        overall_status_by_name[p] = true;
                    }
                }
            }
        }
        for (var menuitem, i = 0; menuitem = popupmenu.childNodes[i]; ++i) {
            var id = menuitem.id;
            var commonPart = "placesContextMenu_sccButton";
            var lastPart = id.substring(commonPart.length);
            lastPart = lastPart.substr(0,1).toLowerCase() + lastPart.substr(1);
            if (overall_status_by_name[lastPart]) {
                menuitem.removeAttribute("disabled");
                var command = ("gPlacesViewMgr.do_scc_multi_item_action('"
                               + lastPart
                               + "', ["
                               + indexInfoArgs.join(", ")
                               + "]);");
    ...
    _disable_all_items: function(popupmenu) {
        for (var menuitem, i = 0; menuitem = popupmenu.childNodes[i]; ++i) {
            menuitem.setAttribute('disabled', 'true');
        }
    },

    _determineItemSCCSupport: function(fileObj, sccObj, isFolder) {
        // Code taken from peSCC.determineItemSCCSupport
        // XXX: merge these two
        try {
            if (fileObj.sccExclude) {
                return false;
            }
        } catch(ex) {
            dump("places.js:: determineItemSCCSupport(#1) " + ex + "\n");
        }
        // assume the worst =)
        sccObj.isscc_file = !isFolder;
        sccObj.isscc_directory = isFolder;
        sccObj.isscc_collection = false; // not applicable
        sccObj.isscc_container = sccObj.isscc_directory;
        sccObj.isscc_file = sccObj.isscc_file || !sccObj.isscc_container;
        sccObj.has_scc = fileObj.sccType != '';
        sccObj.is_scc = sccObj.isscc_file || sccObj.isscc_container;
        if (!sccObj.is_scc) {
            return false;
        }

        sccObj.can_scc = sccObj.is_scc || sccObj.has_scc || (fileObj.sccDirType != '');
        sccObj.missing = !fileObj.exists;
        if (!sccObj.can_scc && !sccObj.missing) {
            sccObj.can_scc = sccObj.has_scc || fileObj.sccDirType;
        }

        try {
            if (fileObj.sccExclude) return false;
        } catch(ex) {
            dump("places.js:: determineItemSCCSupport(#2) " + ex + "\n");
        }
        return true;
    },
    ...
    do_scc_action: function(action, index, isFolder) {
        // Piggyback on the code in peSCC.p.js by
        // creating an "scc_item" object that will satisfy it.
        var uri = index == -1 ? ko.places.manager.currentPlace : this.view.getURIForRow(index);
        var fileObj = (Components.classes["@activestate.com/koFileService;1"].
                       getService(Components.interfaces.koIFileService).
                       getFileFromURI(uri));
        ...
        var command = "cmd_SCC" + action;
        if (isFolder) {
            command += "_folder";
        }
        ko.projects.SCC.doCommand(command, [item]);
    },
    
    do_scc_multi_item_action: function(action, indexAndFolderList) {
        // See comment above in do_scc_action for info on "items"
        var items = [];
        var index, isFolder;
        for (var i = 0; i < indexAndFolderList.length; i++) {
             [index, isFolder] = indexAndFolderList[i];
            var uri = index == -1 ? ko.places.manager.currentPlace : this.view.getURIForRow(index);
            var fileObj = (Components.classes["@activestate.com/koFileService;1"].
                           getService(Components.interfaces.koIFileService).
                           getFileFromURI(uri));
        ...
        ko.projects.SCC.doCommand(command, items);
    },
