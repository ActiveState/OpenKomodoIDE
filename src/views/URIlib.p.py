...
# #if WITH_DEBUGGING
# Need to do this so we can correctly parse
# uri's given to us by the debugger
addSchemeToParser('kodebugger')
addSchemeToParser('dbgp')
# #endif
addSchemeToParser('macro')
addSchemeToParser('snippet')
addSchemeToParser('printdebug')
...
class URIParser(object):
    ...
    def get_displayPath(self):
        ...
        elif self.scheme == 'dbgp':
            # Drop the guid and 'file://' parts from the URL.
            return "dbgp://" + self.get_path()[7:]
        else:
            return self.get_URI()
...
class FileHandlerBase(object):
    ...
    isNetworkFile = False
    # Used by the publishing components to show status changes.
    publishingStatus = 0
...
class projectURI2_Handler(projectURIHandler):
    ...
    def close(self):
        ...
        try:
            if self.part and self._mode and self._mode[0] == 'w':
                val = self._file.getvalue()
                isLogic = self._uri.scheme == "tutorial" and self._uri.path.endswith(".js")
                if isLogic:
                    origVal = self.part.getStringAttribute("logic")
                else:
                    origVal = self.part.value
                if val != origVal:
                    tool = self.getTool()
                    if isLogic:
                        tool.setStringAttribute("logic", val)
                        self.part.setStringAttribute("logic", val)
                    else:
                        tool.value = val
                        self.part.value = val
                    try:
    ...
    def open(self, mode):
        ...
        try:
            ...
            if mode and mode[0] == 'r':
                try:
                    self.part = self.getTool()
                    self._stats = None
                    if self._uri.scheme == "tutorial" and self._uri.path.endswith(".js"):
                        text = self.part.getStringAttribute("logic")
                    else:
