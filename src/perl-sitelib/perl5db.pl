...
# Debugger for Perl 5.6.x; perl5db.pl patch level:
...
if ($notty) {
  ...
  if (($^O ne 'MSWin32') and (! defined $ENV{LINES})) {
      $ENV{LINES} = 24;
  }
  if (($^O ne 'MSWin32') and (! defined $ENV{COLUMNS})) {
      $ENV{COLUMNS} = 80;
  }
  ...
  print "remoteport = $remoteport\n"
    if (defined $remoteport && 0);

  if (defined $remoteport) {
      require IO::Socket;
      
      # Loop waiting for a connection, to give the debugger a chance to catch up
      for (1..10) {
	...
	sleep 1;
      ...
      if (!$OUT) { die "Could not create socket to connect to $remoteport [$!]"; }
      $IN = $OUT;
  }
  else {
    if (defined $console) {
		...
		push(@hist,$cmd) if length($cmd) > 1;
	      PIPE: {
			...
			if (!defined $main::{'_<' . $file}) {
			    if (($try) = grep(m#^_<.*\Q$file\E#, keys %main::)) {{
					      $try = substr($try,2);
...
sub eval {
    ...
    eval { &DB::save };
    if ($at =~ /Use of uninitialized value in/) {
      # print STDERR "Matched uninit value\n";
        # Do nothing
    } elsif ($at) {
      # print STDERR "Problem, not the uninit value\n";
	print STDERR $at;
...
sub readline {
  ...
  if (ref $OUT and UNIVERSAL::isa($OUT, 'IO::Socket::INET')) {
    my $str = join '', @_;
    $OUT->write($str, length $str);
    my $stuff;
    $IN->recv( $stuff, 2048 );  # XXX: what's wrong with sysread?
    # print STDERR "perl5db.pl: QQQ: cmd='$stuff'\n";
    if ($stuff =~ /\n./s) {
	$stuff =~ s/^.*\n([^\n]*\n)$/$1/s;
	# print STDERR "perl5db.pl: QQQ: after trimming missed lines, cmd='$stuff'\n";
    }
    $stuff = "q" unless length $stuff;
    $stuff;
  }
  else {
    $term->readline(@_);
...
sub parse_options {
    ...
    my %opt_needs_val = map { ( $_ => 1 ) } qw{
        ...
        pager quote ReadLine recallCommand
	CallKomodo RemotePort ShellBang TTY
    };
    $opt_needs_val['ReadLine'] = 0;
    while (length) {
	my $val_defaulted;
	s/^\s+// && next;
