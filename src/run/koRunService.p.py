...
# #if WITH_DEBUGGING
                ...
                # get data from the debug listener service
                dbgpManager = components.classes["@activestate.com/koDBGPManager;1"]\
                         .getService(components.interfaces.koIDBGPManager)
                
                if name =="port":
                    value = str(dbgpManager.port)
                elif name =="address":
                    value = dbgpManager.address
                elif name =="proxyPort":
                    value = str(dbgpManager.proxyPort)
                elif name =="proxyAddress":
                    value = dbgpManager.proxyAddress
                elif name =="proxyClientPort":
                    value = str(dbgpManager.proxyClientPort)
                elif name =="proxyClientAddress":
                    value = dbgpManager.proxyClientAddress
                elif name =="proxyKey":
                    value = self._prefs.getStringPref("dbgpProxyKey");
                else:
                    errmsg = "Unable to get value for '%s'" % (name)
                    self.lastErrorSvc.setLastError(nsError.NS_ERROR_INVALID_ARG,
                                                errmsg)
                    raise ServerException(nsError.NS_ERROR_INVALID_ARG, errmsg)
# #endif
...
class KoRunService:
    ...
    def Run(self, command, cwd, env, console, input=None):
        ...
        if not env:
            env = ""
        envDict = self._getEnvDictFromString(env)
        # Cheap hack to avoid env merging, because anything more involved is too prone to breakage this late in the game
        if "_DO_NOT_MERGE_ENV" not in envDict:
            envDict = self._mergeEnvWithParent(envDict)
        # The process library requires that all env strings be ASCII
        # or all Unicode. We'll just make them all Unicode for now.
        #XXX This unicode conversion may not be necessary since
        ...
        if console:
            ...
            #fin.close()
            #print "RUN: ---------------------------------"
            log.debug("Running command: %s, env: %s", actualCommand, json.dumps(envDict))
