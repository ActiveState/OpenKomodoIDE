...
(function() {
     ...
     *  
     * @param   {param} variable The variable to look up
     * @returns {String} The value of the variable if found
     */
    this.lookupVar = function(name)
    {
        var vars = this.getEnv();
        for (var v in vars) 
        {
            if (v == name)
                return vars[v];
    ...
    var showOutputInHud = function(process, command)
    {
        ...
        $($.create("panel", {class: "hud shell-output", noautohide: true, width: 500, level: "floating"},
            $.create("textbox", {id: "hud-shell-output", multiline: true, rows: 15, readonly: true, style: "max-width: 490px"})
                    ("button", {label: "stop"})
        ).toString());
