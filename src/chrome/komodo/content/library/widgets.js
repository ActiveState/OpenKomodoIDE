...
(function() {
    ...
    this.getWidget = function koWidgetManager_getWidget(aID, aForceLoad) {
        ...
        if (aForceLoad && (!data.browser || data.browser.currentURI.spec == "about:blank")) {
            ...
            let pane = this.getPaneAt(widgetMetaData.paneID);
            if (!pane) {
                 pane = this.getPaneAt(data.defaultPane || this.panes[0]);
            }
            if (!data.browser) {
    ...
    this.restoreLayout = function koWidgetManager_restoreLayout(prefs,
                                                                prefpath,
                                                                forceRestore) {
        if(typeof(forceRestore) == undefined){
            forceRestore = false;
        } else {
            log.info("Forcing restore of widgets.");
        }
        if (layoutRestored && !forceRestore) {
