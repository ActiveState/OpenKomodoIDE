...
this.toggleToolbars = function uilayout_toggleToolbars(toolboxId = "main-toolboxrow-wrapper")
{
    ...
    this.setToolbarsVisibility(showToolbars, toolboxId);
}
...
this.setToolbarsVisibility = function uilayout_setToolbarsVisibility(showToolbars, toolboxId = "main-toolboxrow-wrapper")
{
    ...
    if (toolboxId == "toolbox_side")
    {
        var broadcaster = document.getElementById('cmd_toggleSideToolbar');
        broadcaster.setAttribute("checked", showToolbars);
    }
...
function _Observer ()
{
    ...
    window.addEventListener('debugger_state_changed',
                            this.handle_project_changed, false);
...
_Observer.prototype.destroy = function()
{
    ...
    window.removeEventListener('current_project_changed',
                               this.handle_project_changed, false);
    window.removeEventListener('debugger_state_changed',
                               this.handle_project_changed, false);
...
// #if WITH_DEBUGGING
    var debug;
    try {
        // possible that ko.dbg has shut down, but this hasn't
        debug = ko.dbg.manager.getDebugTitle();
    } catch(e) {
        if (e.message != "ko.dbg.manager has no properties"
            && e.message != "ko.dbg.manager is null") {
            _log.exception(e);
        }
        debug = "";
    }
...
function uilayout_upgrade() {
    ...
    removePersistAttributes("sccToolbar",
                            ['collapsed','kohidden', 'hidden']);
