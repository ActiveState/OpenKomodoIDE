...
function test_sccdialog_commit()
{
    var urls, result, i;

    dump("\n-------------------\nsccdialog_commit()\n");
    urls = ["file:///D:/trentm/as/Apps/Komodo-devel/src/version.txt",
            "file:///D:/trentm/as/Apps/Komodo-devel/Construct",
           ];
    result = sccdialog_commit(urls, "default checkin message");
    dump("\tresult: "+result+"\n");
    if (result != null) {
        dump("\t\tmessage: '"+result.message+"'\n");
        for (i = 0; i < result.selected.length; ++i) {
            dump("\t\turl "+i+": "+result.selected[i]+"\n");
        }
    }
}
