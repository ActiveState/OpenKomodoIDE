...
this.onload = (function NotificationsWidgetController_onload() {
  ...
  window.focusNotificationByID = this.focusNotificationByID;
}).bind(this);
...
/**
 * Try to focus (and expand) a given notification, collapsing others
 * @param focusNotificationByID {koINotification} The notification to focus
 */
this.focusNotificationByID = (function NWC_focusNotificationByID(id) {
  var elem = null;
  for each (var child in this.container.children) {
    if (id == child.notification.identifier) {
      this.focusNotification(child.notification);
      break;
    }
  }
}).bind(this);
