...
const spellCheckEngine = Components.classes["@mozilla.org/spellchecker/engine;1"]
...
function docSyntaxCheckingOnLoad() {
    try {
        ...
        dialog.spellCheckDelay = document.getElementById("spellCheckDelay");
        dialog.editUseSpellChecking = document.getElementById("editUseSpellChecking");
        dialog.spellCheckDictionary = document.getElementById("spellCheckDictionary");
        dialog.spellCheckIgnoredWords = document.getElementById("spellCheckIgnoredWords");
        parent.initPanel();
    } catch(e) {
        log.exception(e);
    }
...
function pref_spellcheck_doEnabling() {
    var enabled = dialog.editUseSpellChecking.checked;
    pref_setElementEnabledState(dialog.spellCheckDelay, enabled);
}
...
/** Populates the dictionary list with known dictionaries. */
function populateDictionaries() {
    var dictionaries = {};
    spellCheckEngine.getDictionaryList(dictionaries, {});
    var dictionary = g_prefset.getString(PREF_DICTIONARY, dictionaries.value[0]);
    log.debug("The preferred dictionary is " + dictionary);
    var spellCheckDictionary = document.getElementById("spellCheckDictionary");
    spellCheckDictionary.removeAllItems();
    for (let i in dictionaries.value) {
        let dict = dictionaries.value[i];
        log.debug("Found dictionary " + dict);
        spellCheckDictionary.appendItem(dict, dict);
        if (dict == dictionary) {
            spellCheckDictionary.selectedIndex = i;
        }
    }
}
