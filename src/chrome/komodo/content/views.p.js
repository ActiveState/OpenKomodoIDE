                                                 ...
                                                 index /* =-1 */)
{
    ...
    // XXX remove line excess saveInProg and restoreInProg checks when
    // workspace2 becomes default
    if (ko.workspace.restoreInProgress() ||
        ko.workspace2.restoreInProgress() ) {
        window.addEventListener("komodo-ui-started", function() {
            // CITODO: Scan document 
...
viewManager.prototype.cacheCommandData = function(view)
{
    ...
    if (view) {
        ...
        if (view.koDoc) {
            cache.sccSummary = view.koDoc.isUntitled?'':view.koDoc.file.sccSummary;
            ...
            if (view.koDoc.languageObj) {
                ...
                cache.canDebug = true; // XXX view.koDoc.languageObj.debuggable;
            ...
            for (var u=0; u < urllist.length; ++u) {
                ...
                for (var i=0; i < views.length; ++i) {
                    ...
                    if (views[i] == this.currentView) {
                        window.setTimeout(window.updateCommands, 1, 'SCC');
...
viewManager.prototype.updateCommands = function() {
    ...
    if (update_editor_change || oldcache.canDebug != newcache.canDebug) {
        window.setTimeout(window.updateCommands, 1, 'debuggability_changed');
    }
...
viewManager.prototype.handle_view_closed = function() {
    ...
    // XXX remove line excess saveInProg and restoreInProg checks when
    // workspace2 becomes default
    if (!ko.workspace.saveInProgress() &&
        !ko.workspace2.saveInProgress() &&        
...
viewManager.prototype.handle_view_opened = function() {
    ...
    // workspace2 becomes default
    if (!ko.workspace.saveInProgress() &&
        !ko.workspace.restoreInProgress() &&
        !ko.workspace2.saveInProgress() &&
        !ko.workspace2.restoreInProgress() &&
        !ko.views.manager.batchMode &&
        !ko.main.windowIsClosing)
...
viewManager.prototype.is_cmd_linkViews_supported = function() {
    return 1;
}

viewManager.prototype.is_cmd_linkViews_enabled = function() {
    return require("ko/views").isSplit() && (require('ko/views').current().get().linkedView == null);
}

viewManager.prototype.do_cmd_linkViews = function() {
    if (require("ko/views").isSplit())
        require("ko/views").getLeftView().linkedView = require("ko/views").getRightView();
}

viewManager.prototype.is_cmd_unlinkViews_supported = function() {
    return 1;
}

viewManager.prototype.is_cmd_unlinkViews_enabled = function() {
    return !(require('ko/views').current().get().linkedView == null)
}

viewManager.prototype.do_cmd_unlinkViews = function() {
    require('ko/views').current().get().linkedView = null
}
...
viewManager.prototype.is_cmd_save_enabled = function() {
    return this.currentView && this.currentView.koDoc && (this.currentView.koDoc.isDirty || this.currentView.koDoc.isCollab());
}
...
// #if WITH_DEBUGGING
        if (uri.indexOf("dbgp:")==0) {
            var fileIndex = uri.indexOf("/file://");
            if (fileIndex > 0 && ko.dbg.manager.currentSession) {
                var koFileEx = Components.classes["@activestate.com/koFileEx;1"]
                                     .createInstance(Components.interfaces.koIFileEx);
                koFileEx.URI = uri.substring(fileIndex + 1);
                // find out if we have a mapping
                if (!koFileEx.server) {
                    // be sure we have a hostname in this uri
                    koFileEx.server = ko.dbg.manager.currentSession.dbg.hostname;
                    uri = koFileEx.URI;
                }
            }
        }
